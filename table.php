...
include "connection.php";
   ...
   $stmt = $conn->prepare("SELECT * FROM day_table WHERE course = :subName  AND semester = $sem AND `level` = $level ");
          $stmt->bindParam(":subName", $course);
          $stmt->execute();
    $ROWS = $stmt->rowCount();
...
if ($ROWS > 0) {
   ...
   $monCnt = $mondayRows;
    ...
    echo "<tr><td colspan='3' style='text-align: center;'>No subjects available or  Click on generate before view</td></tr>";
}
